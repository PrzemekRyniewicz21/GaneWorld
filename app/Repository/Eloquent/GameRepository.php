...
class GameRepository implements GameRepositoryInterface
{
    ...
    public function filterBy(?string $phrase, string $type = self::TYPE_DEFAULT, int $limit = 15)
    {
        ...
        if ($type !== self::TYPE_ALL) {
            ...
            $typeExsistInArray = false;
            $exsistingTypes = $this->gameModel
                ->select('type')
                ->distinct()
                ->get();

            foreach ($exsistingTypes as $key) {
                if ($key->getOriginal()['type'] === $type) {
                    $typeExsistInArray = true;
                }
            }
            if (!$typeExsistInArray) {
    ...
    public function stats()
    {
        return [
            ...
            'avg' => round($this->gameModel->avg('metacritic_score') ?? 0, 2),
        ];
    }
